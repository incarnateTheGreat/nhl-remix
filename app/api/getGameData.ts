...
export default async function getGameData(gameId: string) {
  try {
    ...
    if (rightRail?.gameVideo) {
      if (rightRail?.gameVideo?.threeMinRecap) {
      ...
      if (rightRail?.gameVideo?.condensedGame) {
        ...
        if (condensedGameData) {
          rightRail.gameVideo.condensedGameVideoObject = {
            poster: condensedGameData.poster,
            videoUrl:
              condensedGameData.sources[condensedGameData.sources.length - 1]
                .src,
          };
        }
      }
